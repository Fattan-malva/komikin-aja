...
export default function BookmarksPage() {
  ...
  // ✅ PERUBAHAN 1: Dynamic grid rows
  // Jika hanya 1 bookmark → 1 baris, selain itu → 2 baris
  const gridRowsClass = bookmarks.length === 1 ? "grid-rows-1" : "grid-rows-2";

  return (
    <div className="min-h-screen bg-[#0f0f1a] text-white p-6 rounded-lg">
      {/* ✅ PERUBAHAN 2: space-y-8 → space-y-6 */}
        ...
        <section>
          ...
          {bookmarks.length > 0 ? (
            <div className="relative">
              ...
              >
                {/* ✅ PERUBAHAN 1 diterapkan: gridRowsClass dinamis */}
                <div
                  className={`grid ${gridRowsClass} grid-flow-col gap-4 auto-cols-[160px] sm:auto-cols-[200px]`}
                >
                  {bookmarks.map((item) => (
